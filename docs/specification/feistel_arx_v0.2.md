...
**Normative serialization for v0.2 – endianness fixed**

* Plaintext and ciphertext are 16-byte arrays. The block is interpreted as two consecutive 64-bit little-endian words: `L` occupies bytes 0-7, `R` occupies bytes 8-15. The reference implementation uses `u64::from_le_bytes` / `to_le_bytes` for each half.
* The master key is a 32-byte array fed to SHAKE256 as raw bytes with no endian conversion.
* Round keys are derived as 64-bit little-endian values: each 8-byte chunk from SHAKE256 is interpreted as `u64::from_le_bytes`. The round key is then treated as eight bytes `k0..k7` where `ki` is the i-th least-significant byte, i.e. `ki = (K >> (8*i)) & 0xff`.
* Within a 64-bit word, byte ordering for the S-box layer is little-endian: for input `x`, `xi = (x >> (8*i)) & 0xff` for `i = 0..7`. The same ordering applies to the round key bytes.

This fixes endianness for interoperability.
...
The byte ordering is normative little-endian as defined in Section 3: `xi` is the i-th least-significant byte of `x`, i.e. `xi = (x >> (8*i)) & 0xff` for i = 0..7, and similarly for `k`.
...
# 26. Normative Serialization

This section finalizes the serialization and byte ordering for Herringfish Feistel ARX v0.2 to ensure interoperability.

## 26.1 Block serialization

* Plaintext and ciphertext are 16-byte octet strings.
* The block is split into two 64-bit halves: `L` = bytes 0-7, `R` = bytes 8-15.
* Each half is interpreted as an unsigned 64-bit little-endian integer. The reference implementation uses `u64::from_le_bytes` to read and `to_le_bytes` to write.

## 26.2 Key serialization

* The master key is a 32-byte octet string. It is provided to SHAKE256 as raw bytes with no endian transformation.
* Round keys are derived as 64-bit little-endian integers from the SHAKE256 XOF. Each round key `Ki` is the next 8 bytes of the XOF interpreted via `u64::from_le_bytes`.

## 26.3 Round function byte ordering

* For a 64-bit value `x`, the i-th byte used in the S-box layer is `xi = (x >> (8*i)) & 0xff` for `i = 0..7`, i.e., little-endian byte order with `x0` as least-significant byte.
* The same ordering applies to the round key bytes `ki`.
* The diffusion layer operates on the eight bytes in this order.

This normative serialization removes all ambiguity in cross-implementation testing.

# 27. Normative Freeze Summary

Herringfish Feistel ARX v0.2 is frozen for research evaluation with the following normative parameters:

* Block size: 128 bits
* Master key size: 256 bits
* Rounds: 16
* S-box: `HERRINGFISH_SBOX_V02`, affine parameters `a = 0x11`, `b = 0x71`, counter 0
* Key schedule: SHAKE256 XOF with domain `HERRINGFISH-FEISTEL-KEY`, 1024 bits of round key material
* Serialization: little-endian 64-bit halves for block and round keys as defined in Section 26
* Known-answer test vectors: `docs/tables/kat_vectors_v02.txt` and `docs/tables/kat_expanded_v02.txt`

Changes to any normative parameter constitute a new version. The current specification is experimental and must not be used for production security.
