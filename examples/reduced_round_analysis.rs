...
fn encrypt_rounds(key: &[u8;32], pt: &[u8;16], rounds: usize) -> [u8;16] {
    ...
    for i in 0..rounds {
        ...
        let mut bytes = [0u8;8];
        for j in 0..8 { bytes[j] = ((t >> (8*j)) & 0xff) as u8; }
        let mut out_bytes = [0u8;8];
        for j in 0..8 {
            out_bytes[j] = bytes[j] ^ bytes[(j+1)%8] ^ bytes[(j+3)%8];
        }
        let mut f_out = 0u64;
        for j in 0..8 { f_out |= (out_bytes[j] as u64) << (8*j); }
...
fn estimate_max_prob(rounds: usize, samples: usize) -> (f64, f64) {
    ...
    for bit in 0..8 {
        ...
        for _ in 0..samples {
            ...
            let mut diff = [0u8;16];
            for i in 0..16 { diff[i] = o1[i] ^ o2[i]; }
...
fn main() {
    let samples = 100000;
    println!("Reduced-round differential max prob, samples per bit = {}", samples);
    for rounds in [4,6,8,12] {
        ...
        println!("Rounds {}: max prob ≈ {:.6} 95% CI [{:.6}, {:.6}]", rounds, p, ci_low, ci_high);
    }
}
