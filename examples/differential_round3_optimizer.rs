...
impl Ddt {
    fn build() -> Self {
        ...
        let mut transitions =
            vec![Vec::<ByteTransition>::new(); BYTE_VALUES];
    ...
    fn print_statistics(&self) {
        ...
        println!(
            "Global minimum byte weight : {:.6}",
            self.global_min_weight
        );
    }
...
impl<'a> FSearch<'a> {
    ...
    fn partial_future_f_lb(
        ...
        for i in 0..BYTE_COUNT {
            ...
            if !assigned[i]
                || !assigned[dependency_1]
                || !assigned[dependency_3]
            {
                continue;
            }
            ...
            let next_byte =
                get_byte(self.fixed_xor, i) ^ df_byte;
        ...
        match arg.as_str() {
            ...
            "--result-limit" => {
                result_limit =
                    args.next()
                        .expect(
                            "missing value for --result-limit",
                        )
                        .parse::<usize>()
                        .expect(
...
#[inline]
fn round2_candidate_lb(
    ddt: &Ddt,
    state: &DifferentialState,
) -> f64 {
    state.weight
        + ddt.f_input_lb(state.dr)
}
...
fn generate_round2(
    ddt: &Ddt,
    round1: &[DifferentialState],
    config: &Config,
) -> Round2Result {
    let mut beam =
        BinaryHeap::<Candidate>::new();
...
impl PartialEq for Characteristic {
    fn eq(&self, other: &Self) -> bool {
        self.states.iter().zip(other.states.iter()).all(|(a, b)| {
            a.dl == b.dl && a.dr == b.dr && a.weight == b.weight
        })
    }
}
...
fn main() {
    ...
    println!();
    println!(
        "Target-aware branch-and-bound search for"
    );
    println!(
        "individual three-round differential characteristics."
    );
    ...
    println!(
        "Maximum F transitions : {}",
        config.f_max
    );

    println!(
        "Result retention limit : {}",
        config.result_limit
    );
    ...
    println!(
        "F transitions rejected  : {}",
        r1_stats.heap_rejected
    );

    println!(
        "F transitions evicted  : {}",
        r1_stats.heap_evicted
    );

    println!(
        "F lower-bound pruned    : {}",
        r1_stats.lower_bound_pruned
    );

    println!(
        "F target-pruned         : {}",
        r1_stats.target_pruned
    );
        ...
        round2.entries.first()
    {
        ...
        println!(
            "Worst retained W             : {:.6}",
            worst
                .map(|x| x.state.weight)
                .unwrap_or(best.state.weight)
        );
    ...
    println!();
    println!(
        "Round-2 lower-bound diagnostics:"
    );
    ...
    println!(
        "Round-2 states considered       : {}",
        round2.entries.len()
    );

    println!(
        "Pruned by lower bound           : {}",
        round3.pruned
    );
    ...
    if !round3.characteristics.is_empty() {
        let display_count =
            round3.characteristics.len().min(25);

        println!();
        println!(
            "Top {} Round-3 characteristics:",
            display_count
        );

        println!();

        for (index, characteristic)
            in round3.characteristics
                .iter()
                .take(display_count)
                .enumerate()
        {
            let final_state =
                characteristic.states[3];
        ...
        let best =
            &round3.characteristics[0];

        println!(
            "============================================================"
        );
        println!(
            "BEST OBSERVED CHARACTERISTIC"
        );
        println!(
            "============================================================"
        );
        ...
        println!(
            "Weight        : {:.6}",
            best.states[3].weight
        );

        println!(
            "Probability   : {:.16e}",
            best.states[3].probability()
        );
        ...
        println!(
            "RESULT: NO CHARACTERISTIC FOUND"
        );
        println!(
            "============================================================"
        );
        ...
        println!(
            "No retained characteristic satisfies:"
        );
    ...
    println!(
        "  Target lower bounds are safe."
    );

    println!(
        "  Per-byte DDT minimums are safe."
    );

    println!(
        "  Partial diffusion bounds only use fully known bytes."
    );

    println!(
        "  Round-3 bounds account for ΔL XOR ΔF cancellation."
    );
    ...
    println!(
        "It does NOT compute differential-hull probabilities."
    );
    ...
    if round2.beam_truncated
        || round2.f_truncated
        || round3.stats.expansions_truncated != 0
    {
        println!(
            "RESULT STATUS: BOUNDED SEARCH"
        );

        println!();

        println!(
            "At least one configured search or retention limit restricted"
        );
        ...
        println!(
            "The best characteristic found is therefore"
        );

        println!(
            "the best characteristic within the searched"
        );
        ...
        println!(
            "A global optimum requires exhaustive coverage."
        );
    } else {
        println!(
            "RESULT STATUS: EXHAUSTIVE WITHIN GENERATED STATE SPACE"
        );

        println!();

        println!(
            "No configured beam or F-transition limit"
        );
        ...
        println!(
            "The target-bound recursion was therefore"
        );

        println!(
            "exhaustive over the generated differential"
        );
        ...
        if round3.results_truncated {
            ...
            println!(
                "The result list was truncated, but the retained top results"
            );

            println!(
                "still include the globally best characteristic from that search."
            );
        }
    }
}
...
mod tests {
    ...
    fn final_round_does_not_charge_for_a_fourth_round() {
        ...
        assert!(
            result.characteristics[0].states[3].weight <= 37.0 + EPS
        );
    }
