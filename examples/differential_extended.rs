#![allow(clippy::all, dead_code, unused_imports, unused_variables, unused_assignments)]
use rand::{Rng, SeedableRng};
use sha3::digest::{ExtendableOutput, Update};
use shake::Shake256;
use std::collections::HashMap;
use std::convert::TryInto;

const FEISTEL_ROUNDS: usize = 16;
const KEY_SIZE: usize = 32;

fn f_function(x: u64, k: u64, sbox: &[u8; 256]) -> u64 {
    let mut out = 0u64;
    for i in 0..8 {
        let x_byte = ((x >> (8 * i)) & 0xff) as u8;
        let k_byte = ((k >> (8 * i)) & 0xff) as u8;
        let sb = sbox[(x_byte ^ k_byte) as usize];
        out |= (sb as u64) << (8 * i);
    }
    // diffusion inside F
    let mut bytes = [0u8; 8];
    for i in 0..8 {
        bytes[i] = ((out >> (8 * i)) & 0xff) as u8;
    }
    let mut out_bytes = [0u8; 8];
    for i in 0..8 {
        out_bytes[i] = bytes[i] ^ bytes[(i + 1) % 8] ^ bytes[(i + 3) % 8];
    }
    let mut out2 = 0u64;
    for i in 0..8 {
        out2 |= (out_bytes[i] as u64) << (8 * i);
    }
    out2
}

fn derive_round_keys(key: &[u8; 32]) -> Vec<u64> {
    const DOMAIN: &[u8] = b"HERRINGFISH-FEISTEL-KEY";
    let mut hasher = Shake256::default();
    hasher.update(DOMAIN);
    hasher.update(key);
    let mut out = vec![0u8; FEISTEL_ROUNDS * 8];
    hasher.finalize_xof_into(&mut out);
    (0..FEISTEL_ROUNDS)
        .map(|i| {
            let mut b = [0u8; 8];
            b.copy_from_slice(&out[i * 8..i * 8 + 8]);
            u64::from_le_bytes(b)
        })
        .collect()
}

fn derive_sbox() -> [u8; 256] {
    const SBOX: [u8; 256] = [
        ,
    ];
    SBOX
}

fn feistel_encrypt(key: &[u8; 32], pt: &[u8; 16], rounds: usize, sbox: &[u8; 256]) -> [u8; 16] {
    let keys = derive_round_keys(key);
    let mut left = u64::from_le_bytes(pt[0..8].try_into().unwrap());
    let mut right = u64::from_le_bytes(pt[8..16].try_into().unwrap());
    for i in 0..rounds {
        let f_out = f_function(right, keys[i], sbox);
        let new_right = left ^ f_out;
        left = right;
        right = new_right;
    }
    let mut out = [0u8; 16];
    out[0..8].copy_from_slice(&left.to_le_bytes());
    out[8..16].copy_from_slice(&right.to_le_bytes());
    out
}

fn hamming_weight(x: u128) -> u32 {
    x.count_ones()
}

fn generate_diff(hw: u32) -> [u8; 16] {
    let mut diff = [0u8; 16];
    let mut bits = hw as usize;
    let mut pos = 0;
    while bits > 0 {
        diff[pos / 8] |= 1 << (pos % 8);
        pos += 1;
        bits -= 1;
    }
    diff
}

fn estimate_max_prob(rounds: usize, samples: usize, hw: u32) -> f64 {
    let key = [0u8; 32];
    let sbox = derive_sbox();
    let mut rng = rand::rngs::StdRng::seed_from_u64(0x12345678);
    let diff = generate_diff(hw);
    let mut freq: HashMap<[u8; 16], usize> = HashMap::new();
    for _ in 0..samples {
        let mut p = [0u8; 16];
        rng.fill_bytes(&mut p);
        let mut p2 = p;
        for i in 0..16 {
            p2[i] ^= diff[i];
        }
        let o1 = feistel_encrypt(&key, &p, rounds, &sbox);
        let o2 = feistel_encrypt(&key, &p2, rounds, &sbox);
        let mut d = [0u8; 16];
        for i in 0..16 {
            d[i] = o1[i] ^ o2[i];
        }
        *freq.entry(d).or_insert(0) += 1;
    }
    let max_count = freq.values().cloned().max().unwrap_or(0);
    max_count as f64 / samples as f64
}

fn main() {
    let samples = 100_000;
    println!("Differential sampling extended, samples = {}", samples);
    for rounds in [4, 6, 8, 12] {
        for hw in [1u32, 2, 4] {
            let p = estimate_max_prob(rounds, samples, hw);
            let se = (p * (1.0 - p) / samples as f64).sqrt();
            let ci_low = (p - 1.96 * se).max(0.0);
            let ci_high = (p + 1.96 * se).min(1.0);
            println!(
                "Rounds {} HW {}: max p̂ = {:.6} 95% CI [{:.6},{:.6}]",
                rounds, hw, p, ci_low, ci_high
            );
        }
    }
}
