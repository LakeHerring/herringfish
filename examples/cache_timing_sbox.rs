...
fn main() {
    ...
    for i in 0..256 { black_box(HERRINGFISH_SBOX_V02[i]); }
    ...
    let var_seq = seq_times.iter().map(|x| (x - mean_seq).powi(2)).sum::<f64>() / (TRIALS - 1) as f64;
    let var_rand = rand_times.iter().map(|x| (x - mean_rand).powi(2)).sum::<f64>() / (TRIALS - 1) as f64;
    ...
    println!("Sequential: mean = {:.1} ns, std = {:.1} ns", mean_seq / ITER_PER_TRIAL as f64, std_seq / ITER_PER_TRIAL as f64);
    println!("Random:     mean = {:.1} ns, std = {:.1} ns", mean_rand / ITER_PER_TRIAL as f64, std_rand / ITER_PER_TRIAL as f64);
    ...
    println!("Table lookup is secret-dependent. Use encrypt_block_ct for constant-time evaluation.");
}
