...
fn main() {
    ...
    let var_pair = pairwise_dist.iter().map(|d| (*d as f64 - mean_pair).powi(2)).sum::<f64>() / SAMPLES as f64;
    ...
    let var_rel = related_dist.iter().map(|d| (*d as f64 - mean_rel).powi(2)).sum::<f64>() / SAMPLES as f64;
    
    println!("Key-schedule independence test");
    println!("Samples: {}", SAMPLES);
    println!("Pairwise round-key Hamming distance:");
    println!("  mean = {:.2} bits, std = {:.2}", mean_pair, var_pair.sqrt());
