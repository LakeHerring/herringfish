...
fn main() {
    ...
    for w in [1,2,4] {
        ...
        for _ in 0..1000 {
            ...
            for i in 0..32 { key1[i] = base_key[i] ^ diff[i]; }
            let corrs = round_key_correlation(&base_key, &key1);
            let avg = corrs.iter().sum::<f64>() / corrs.len() as f64;
            total += avg;
            trials += 1;
        }
        println!("Hamming weight {} key diff -> avg round key Hamming distance {:.2}/128 bits", w, total/trials as f64);
    }
    ...
    println!("Random key pairs avg round key Hamming distance {:.2}/128 bits", sum/1000.0);
}
