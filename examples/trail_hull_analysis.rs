...
fn max_ddt_prob(ddt: &[[u16;256];256]) -> f64 {
    ...
    for dx in 1..256 {
        for dy in 0..256 {
            if ddt[dx][dy] > max { max = ddt[dx][dy]; }
        }
    }
    max as f64 / 256.0
}
...
fn main() {
    ...
    for rounds in [6usize, 8usize] {
        ...
        println!("{:<6} | {:<18} | {:.3e}", rounds, count_active(right_active), prob);
    }
    
    println!("\nHull analysis note:");
    println!("A differential hull aggregates all characteristics with same input/output difference.");
    println!("Current heuristic uses per-S-box max probability 4/256.");
    println!("Full hull requires enumeration of all intermediate differences consistent with the input/output pair.");
    println!("For 6-8 rounds, exact hull enumeration is computationally intensive and is left as a research task.");
}
