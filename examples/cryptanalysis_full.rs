...
fn differential_max_prob(key: &[u8; 32], rounds: usize, samples: usize, is_feistel: bool) -> f64 {
    ...
    let mut best = 0.0;
    for bit in 0..8 {
        // sample subset of bits for speed
        let mut pt = base_pt;
        pt[0] ^= 1 << bit;
        ...
        for _ in 0..samples {
            ...
            let out1 = if is_feistel {
                feistel_encrypt_trace(key, &p, rounds)
                    .last()
                    .unwrap()
                    .clone()
            } else {
            ...
            let out2 = if is_feistel {
                feistel_encrypt_trace(key, &p2, rounds)
                    .last()
                    .unwrap()
                    .clone()
            } else {
...
fn main() {
    ...
    println!(
        "F-function sampled max diff count {}/500k = {:.6}",
        max_diff,
        max_diff as f64 / 500_000.0
    );
}
