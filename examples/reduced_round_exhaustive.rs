use shake::Shake256;
use sha3::digest::{Update, ExtendableOutput};
use std::collections::HashMap;

const ROUNDS: usize = 4; // exhaustive feasible
const KEY: [u8;32] = [0u8;32];

fn f_function(x: u64, k: u64, sbox: &[u8;256]) -> u64 {
    let mut out = 0u64;
    for i in 0..8 {
        let x_byte = ((x >> (8*i)) & 0xff) as u8;
        let k_byte = ((k >> (8*i)) & 0xff) as u8;
        let sb = sbox[(x_byte ^ k_byte) as usize];
        out |= (sb as u64) << (8*i);
    }
    let mut bytes = [0u8;8];
    for i in 0..8 { bytes[i] = ((out >> (8*i)) & 0xff) as u8; }
    let mut out_bytes = [0u8;8];
    for i in 0..8 {
        out_bytes[i] = bytes[i] ^ bytes[(i+1)%8] ^ bytes[(i+3)%8];
    }
    let mut out2 = 0u64;
    for i in 0..8 { out2 |= (out_bytes[i] as u64) << (8*i); }
    out2
}

fn derive_round_keys(key: &[u8;32]) -> Vec<u64> {
    const DOMAIN: &[u8] = b"HERRINGFISH-FEISTEL-KEY";
    let mut hasher = Shake256::default();
    hasher.update(DOMAIN);
    hasher.update(key);
    let mut out = vec![0u8; 16*8];
    hasher.finalize_xof_into(&mut out);
    (0..16).map(|i| {
        let mut b = [0u8;8];
        b.copy_from_slice(&out[i*8..i*8+8]);
        u64::from_le_bytes(b)
    }).collect()
}

fn feistel_encrypt(pt: u128, rounds: usize, keys: &[u64], sbox: &[u8;256]) -> u128 {
    let mut left = (pt >> 64) as u64;
    let mut right = pt as u64;
    for i in 0..rounds {
        let f_out = f_function(right, keys[i], sbox);
        let new_right = left ^ f_out;
        left = right;
        right = new_right;
    }
    ((left as u128) << 64) | (right as u128)
}

fn main() {
    const SBOX: [u8;256] = [
        ,
    ];
    let keys = derive_round_keys(&KEY);
    let mut diff_counts: HashMap<u128, HashMap<u128, u64>> = HashMap::new();
    // Limited exhaustive search for 1-bit input difference
    let total = 1u64 << 12; // sample subset for demo
    for i in 0..total {
        let pt = i as u128;
        let pt2 = pt | 1u128;
        let c1 = feistel_encrypt(pt, ROUNDS, &keys, &SBOX);
        let c2 = feistel_encrypt(pt2, ROUNDS, &keys, &SBOX);
        let din = pt ^ pt2;
        let dout = c1 ^ c2;
        diff_counts.entry(din).or_insert_with(HashMap::new).entry(dout).and_modify(|e| *e += 1).or_insert(1);
    }
    for (din, map) in diff_counts {
        let max_cnt = map.values().cloned().max().unwrap_or(0);
        let prob = max_cnt as f64 / total as f64;
        println!("Δin {:#018x} max prob {:.6} count {}", din, prob, max_cnt);
    }
    println!("Exhaustive subset completed for {} plaintext pairs", total);
}
